...
import { parseRetryAfterMs, retryAsync, truncateBody } from '@4cloudguru/pipeline-task-core'
 ...
 */
export class OciTokenExchangeError extends Error {
  constructor(
    message: string,
 ...
 * This is defense-in-depth on top of the caller's setSecret() masking and the
 * length truncation in truncateBody(); it runs BEFORE truncation so a full
 * secret occurrence is scrubbed even when it straddles the truncation boundary.
 * Literal split/join (no regex) avoids any ReDoS or escaping concern; only
 * non-trivial values are scrubbed so short, legitimate diagnostic text is never
 * over-redacted.
 */
...
// creation. Requiring it narrows the realm-suffix check above from "any Oracle
// tenant's identity domain" to "the shape every real identity domain host
// actually has" -- it cannot single out the OPERATOR's own tenant specifically
// (that would need an out-of-band allowlist of the operator's own domain IDs,
// which this package does not have), but it does reject a host that is
// syntactically not an identity-domain host at all, closing the gap between
...
export function validateIdentityDomainUrl(identityDomainUrl: string): URL {
  ...
  // Rejecting a non-empty path was considered and deliberately NOT applied:
  // exchangeOidcForUpst below incorporates validated.pathname into the token
  // endpoint it builds (`base = origin + pathname`, then `+ '/oauth2/v1/token'`),
  // so a path-bearing identity-domain URL is existing, intentional, supported
  // configuration -- not a mistyped input. Rejecting it here would break that
  // path for real operators, not close a gap.
  return parsed
}
...
export async function exchangeOidcForUpst(
  ...
  debug(`Exchanging OIDC JWT for OCI UPST at ${tokenEndpoint}`)

  // Bounded exponential-backoff retry via the shared retry helper, matching the
  // sibling ADO-side TokenGenerator: retry a network error, a timeout, a 5xx, or
  // a 429; never a received other 4xx, a refused redirect, or a malformed/short
  // body. Each attempt gets its own fresh 30s AbortController. A capped
  // Retry-After from a retryable response is honored over the default backoff.
  // The OIDC subject_token is the only secret in the request body; pass it so a
  // reflected-parameter validation-error body is scrubbed before it reaches an
  // unmasked failure message. client_id/public_key are not secret.
  return retryAsync(() => attemptExchange(tokenEndpoint, body.toString(), [oidcToken]), {
    ...
    onRetry: (attempt, delayMs, outcome) => {
      const message =
        outcome.kind === 'error' && outcome.error instanceof Error ? outcome.error.message : ''
      debug(
        `OCI token exchange attempt ${attempt + 1} failed: ${message}. Retrying in ${delayMs}ms...`,
      )
    },
  })
}
...
async function attemptExchange(
  ...
  try {
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new OciTokenExchangeError(
          `Timed out exchanging OIDC token for OCI UPST (30s timeout).`,
          true,
        )
      }
      throw new OciTokenExchangeError(
        `Failed to exchange OIDC token for OCI UPST: ${error instanceof Error ? error.message : error}`,
        true,
      )
    }
    ...
    try {
      ...
      throw new OciTokenExchangeError(
        `OCI token exchange returned a non-JSON response: ${sanitizeBody(bodyText, secretsToScrub)}`,
        false,
      )
    }
    ...
    if (!upst) {
      throw new OciTokenExchangeError(
        'OCI token exchange response missing access_token/token field.',
        false,
      )
    }

    debug('Successfully exchanged OIDC JWT for OCI UPST.')
    return upst
  } finally {
